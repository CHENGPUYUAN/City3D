...
double PolyFitInfo::compute_point_confidences(PointSet* pset, int s1 /* = 6 */, int s2 /* = 16 */, int s3 /* = 32 */)
{
	std::vector<vec3>& points = pset->points();
	std::vector<double>& planar_qualities = pset->planar_qualities();
	...
	for (std::size_t i = 0; i < points.size(); ++i)
	{
		...
		for (int j = 0; j < 3; ++j)
		{
			...
			if (j == 0)
			{
				for (unsigned int i = 1; i < sqr_distances.size(); ++i)
				{ // starts from 1 to exclude itself
				}
			}
		}
		...
		for (int j = 0; j < 3; ++j)
		{
			conf += (1 - 3.0 * eigen_values[j][0] / (eigen_values[j][0] + eigen_values[j][1] + eigen_values[j][2]))
				* (eigen_values[j][1] / eigen_values[j][2]);
		}
		conf /= 3.0;
		planar_qualities[i] = (conf);
		progress.next();
	}
	return (total / points.size());
}

void PolyFitInfo::generate(PointSet* pset, Map* mesh, std::vector<Plane3d*>& v, bool use_conficence /* = false */)
{
	facet_attrib_supporting_vertex_group.bind(mesh, Method::facet_attrib_supporting_vertex_group);
	facet_attrib_supporting_plane.bind(mesh, "FacetSupportingPlane");
	...
	//Logger::out("-") << groups.size()<< " vertex group" << std::endl;
...
//				Logger::err("-") << cir->vertex()->point() << std::endl;
		...
		{
			auto ver = v[k];
			if (facet_attrib_supporting_plane[f] == ver)
			{
				count++;
				break;
			}
...
double PolyFitInfo::facet_points_projected_in(PointSet* pset,
	VertexGroup* g,
	MapTypes::Facet* f,
	double max_dist,
	std::vector<unsigned int>& points)
{
	...
	const std::vector<vec3>& pts = pset->points();
	const std::vector<double>& confidences = pset->planar_qualities();

	points.clear();
	double epsilon = max_dist * 0.5f;// considering noise and outliers
	double count = 0.0f;
	for (int i = 0; i < g->size(); ++i)
	{
		...
		if (Geom::point_is_in_polygon(plg2d, q))
		{
			...
			double dist = std::sqrt(plane.squared_ditance(p));
			if (dist < epsilon)
			{ // in case of numerical issues (floating point precision)
